...
class GMMILFORewardWrapper(Wrapper):
    ...
    def reward_function(self, state_state):
        distances_expert, _ = self.nbrs_expert.kneighbors(state_state.reshape((1, -1)))
    ...
    def buffer_to_points(self, buffer):
        ...
        prev_observations = observations[:-1]
        next_observations = observations[1:]
        points = np.hstack((prev_observations, next_observations))
        return points
...
def train_gmmilfo_actor(
    ...
    w_env = GMMILFORewardWrapper(env, buffer, action_multiplier)
