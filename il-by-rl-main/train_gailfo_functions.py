...
#         super().__init__()
#         self.fc1 = nn.Linear(observation_size * 2, 256)
#         self.fc2 = nn.Linear(256, 256)
#         self.fc3 = nn.Linear(256, 1)
#
#     def forward(self, x, next_x):
#         h = torch.relu(self.fc1(torch.cat([x, next_x], dim=1)))
#         h = torch.relu(self.fc2(h))
#         # return torch.relu(self.fc3(h)) + 1e-6
#         return (torch.tanh(self.fc3(h)) + 1.0) * 0.48 + 0.01
#
#
# class GAIFORewardWrapper(Wrapper):
#     def __init__(
#         self,
#         env,
#         expert_buffer,
...
#             expert_buffer
#         )
#         self.discriminator = Discriminator(observation_size)
...
#             torch.tensor(state).reshape((1, -1)).to(torch.float), torch.tensor(next_state).reshape((1, -1)).to(torch.float)
#         )
...
#             )
#             print(imitator_batch_obs.shape, flush=True)
...
#         il_reward = self.reward_function(self.prev_state, env_state)
