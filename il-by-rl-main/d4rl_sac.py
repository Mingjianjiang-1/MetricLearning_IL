...
class SAC:
    ...
    def update_actor_with_reg(self, obs_t, exp_obs_t, smallds_expert_a_t, rng, alpha=0.5):
        obs_t = torch.tensor(obs_t, device=self.device).float()
        exp_obs_t = torch.tensor(exp_obs_t, device=self.device).float()
        smallds_expert_a_t = torch.tensor(smallds_expert_a_t, device=self.device).float()

        act_t, log_prob = self.actor(obs_t, with_log_prob=True)

        bs = 2000

        # minibatch_indices = rng.choice(exp_obs_t.shape[0], bs, replace=True)
        # obs_t_bc = exp_obs_t[minibatch_indices, :]
        # expert_a_t_bc = smallds_expert_a_t[minibatch_indices, :]
        obs_t_bc = exp_obs_t
        expert_a_t_bc = smallds_expert_a_t
        actor_act_t_bc = self.actor(
            obs_t_bc, deterministic=True, with_log_prob=False
        )
        # target_sigma = 0.01
        per_state_losses = torch.mean((expert_a_t_bc - actor_act_t_bc) ** 2, axis=1)
        bc_loss = torch.mean(per_state_losses)
        # print("BC_LOSS:", bc_loss, flush=True)
        q1_t = self.critic1(obs_t, act_t)
        q2_t = self.critic2(obs_t, act_t)
        q_t = torch.min(q1_t, q2_t)

        loss = bc_loss * alpha - q_t.mean() # (self.log_temp.exp() * log_prob - q_t).mean()
        # loss = - q_t.mean()
        self.actor_optim.zero_grad()
        loss.backward()
        self.actor_optim.step()
